...
defmodule Sneakers23.Inventory.ServerTest do
  ...
  describe "start_link/1" do
    test "the process starts and loads from the loader", %{test: test} do
      {:ok, pid} = Server.start_link(name: test, loader_mod: FakeLoader)
      assert :sys.get_state(pid) == Inventory.new()
    end
  end

  describe "get_inventory/1" do
    test "the inventory is returned", %{test: test} do
      {:ok, pid} = Server.start_link(name: test, loader_mod: FakeLoader)
